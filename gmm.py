...
def find(data, center, mu, sig, pi, all_prob):
    for k in range(3):
        gaussian = multivariate_normal(mean=mu[k],cov=sig[k])
        num = pi[k] * gaussian.pdf(data)
        den=[]
        for p,m,si in zip(pi,mu,sig):
            den.append(p*multivariate_normal(mean=m,cov=si).pdf(data))
        den=np.sum(den,axis=0)
        all_prob[:,k]=num/den
    return center

def update(data, center, mu, sig, pi, all_prob):
    res={}
    clustered_data={}
    for i in range(len(data)):
        if center[i] in res:
            clustered_data[center[i]].append(data[i])
            res[center[i]]+=1
        else:
            clustered_data[center[i]]=[data[i]]
            res[center[i]]=1
    data=np.array(data)
    for i in range(3):
        zg = np.sum(all_prob[:,i], axis=0)
        m = (1/zg)*np.sum(data*all_prob[:,i].reshape(len(data),1),axis=0)
        mu[i]=m
        diff = data-m
        sig[i]=((np.dot((np.array(all_prob[:,i]).reshape(len(data),1)*(diff)).transpose(),(diff))/(zg)))
        pi[i]=(zg/np.sum(all_prob))
    return pi, mu, sig

_x=[]
_x.append([(5.9, 3.2), (4.6, 2.9), (6.2, 2.8), (4.7, 3.2), (5.5, 4.2), (5.0, 3.0),
       (4.9, 3.1), (6.7, 3.1), (5.1, 3.8), (6.0, 3.0)])
_x.append([(3.600, 79),(1.800, 54),(3.333, 74),(2.283, 62),(4.533, 85),(2.883, 55),(4.700, 88),(3.600, 85),
...
_all_mu=[]
_all_mu.append([[6.2, 3.2], [6.6, 3.7], [6.5, 3.0]])
_all_mu.append([[4.0, 81], [2.0, 57], [4.0, 71]])
_all_sig=[]
_all_sig.append([ [ [4, 0], [0, 1] ], [ [4, 0], [0, 1] ], [ [4, 0], [0, 1] ]])
_all_sig.append([ [ [1.30, 13.98], [13.98, 184.82] ], [ [1.30, 13.98], [13.98, 184.82] ], [ [1.30, 13.98], [13.98, 184.82] ]])
pi=[1/3, 1/3, 1/3]
...
for i in range(2):
    ...
    all_mu=_all_mu[i]
    all_sig=_all_sig[i]
    cluster=[np.random.randint(0,2) for i in range(len(x))]
    colors = ['r', 'g', 'b']
    x_all, y_all = zip(*x)
    old_cluster=None
    it=0
    while it<5:
        it+=1
        all_prob=np.zeros((len(x),len(all_sig)))
        new_cluster = find(x.copy(), cluster.copy(), all_mu.copy(), all_sig.copy(), pi.copy(), all_prob)
        new_pi, new_mu, new_sig = update(x.copy(), new_cluster, all_mu.copy(), all_sig.copy(), pi.copy(), all_prob)

        all_mu=new_mu.copy()
        all_sig=new_sig.copy()
        pi=new_pi.copy()
        ...
        if i==1:
            ...
            for j in range(3):
                ...
                plot_cov_ellipse(all_sig[j], all_mu[j], alpha=0.5, color=colors[j])
            plt.savefig('task3_gmm_iter'+str(it)+'.jpg')
            plt.clf()
